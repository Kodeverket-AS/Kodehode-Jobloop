...
export default function Offices({ content }) {
  return (
    <div className="flex flex-col items-center w-full px-8 py-10 md:w-full sm:px-10 md:px-12 lg:px-24">
      ...
      <div className="w-full pt-0 gap-2 pb-10 md:flex md:justify-center xl:max-w-[1088px]">
        {content.map((offices, index) => (
          <div
            key={offices.id + index}
            className="flex flex-row justify-between w-full pb-10 mx-auto 2xs:mx-0 md:flex-col md:w-40 lg:w-48 md:pb-0"
          >
            <Image
              src={offices.image}
              width={1000}
              height={1000}
              className="object-cover w-24 h-full mr-3 border rounded border-kv-white 2xs:w-1/2 md:mb-2 md:w-full md:mr-0"
              alt={offices.alt || ""}
            />
